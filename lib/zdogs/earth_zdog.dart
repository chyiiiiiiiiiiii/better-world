...
class _EarthZDogState extends ConsumerState<EarthZdog> {
  var control = Control.stop; // define variable
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {
        ref.read(homeControllerProvider.notifier).earthClicked();
        _toggleDirection();
      },
                          ...
                          children: [
                            Transform.scale(
                              ...
                              child: ZIllustration(
                                ...
                                children: [
                                  if (leaderMovie.get('translate') < 1)
                                    const ZPositioned(
...
class TriangleTreeZdog extends StatelessWidget {
  const TriangleTreeZdog({super.key, required this.degree});
  final double degree;
  @override
  Widget build(BuildContext context) {
    return ZPositioned(
...
class CircleTreeZdog extends StatelessWidget {
  const CircleTreeZdog({super.key, required this.degree});
  final double degree;
  @override
  Widget build(BuildContext context) {
    return ZPositioned(
